...
TYPE = 0
JUMP_BOX = "10.236.211.60"
JUMP_BOXES_LIST = ["10.236.211.60", "10.236.211.61", "10.236.211.62", "10.236.211.63"]
VM_LISTS = ["172.24.10.213", "172.24.10.214", "172.24.10.215", "172.24.10.216"]
PASSWORDS = ["password", "Scaleio123!", "password", "password"]
        ...
        if TYPE == 1:
            ...
            for i in range(0, 4):
                ...
                my_connection.run_cmd(pane,
                                      f"sshuttle -r root@{JUMP_BOXES_LIST[i]} {VM_LISTS[i]}/16 --no-latency-control")
                time.sleep(2)
                my_connection.enter_password(pane, PASSWORDS[i])
                print(f"A ssh tunnel from vdi to {VM_LISTS[i]} via {JUMP_BOXES_LIST[i]} was created.")
            input("click any-key")
        elif TYPE == 0:
            my_connection = Connection()
            my_connection.create_detached_tmux_session(session_name)
